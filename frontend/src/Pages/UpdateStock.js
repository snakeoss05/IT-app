...
export default function UpdateStock() {
  ...
  const GetPcTable = async () => {
    axios
      .get(
        `http://localhost:5000/api/Matriel/get/filter?groupe=${selectedCategory}&query=${query}&page=${currentPage}&limit=${posteperpage}`
      )
      .then((res) => {
  ...
  const UpdatePc = async (id) => {
    axios
      .put(`http://localhost:5000/api/Matriel/updatePc/${id}`, pcUpdate)
      .then((res) => {
  ...
  const UpdateQuantity = async (id, count) => {
    axios
      .put(`http://localhost:5000/api/Matriel/updateQuantity/${id}`, { count })
      .then((res) => {
        setPctable((entry) =>
          entry.map((data) =>
            data._id === id
  ...
  const deletItem = async (id) => {
    axios
      .delete(`http://localhost:5000/api/Matriel/delete/${id}`)
      .then((res) => {
        console.log(res.data);
